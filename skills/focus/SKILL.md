...
4. **Tell the user plainly:**
   ...
   ```
5. **Commit:** `git add -f .focus/plan.md .focus/log.md && git commit -m "focus: handoff at task N"`. The `-f` is required — Session Start step 6 gitignores plan.md/log.md, and this commit is the one sanctioned exception; without `-f` the add fails as an ignored path. The commit makes the handoff durable across session boundaries, including crashes. **Track Mode: skip this commit entirely** (rule 4) — push the branch and, on a published plan, post the handoff block as an issue comment instead.
...
- Do **not** skip the commit (outside Track Mode, where rule 4 replaces it with a pushed branch + issue comment). An uncommitted handoff vanishes if the session crashes.
