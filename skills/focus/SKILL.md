...
6. When creating `.focus/` for the first time, also create `.focus/.gitignore` with `plan.md`, `log.md`, `report.md`, `.toolcount`, and `.lastsession` (temporary files). `memory.md` and `journal/` are committed.
...
- **Natural boundary:** a LARGE plan's top-level task has just completed, or a Track Mode session block finished (rule 6 — its reason value is `boundary`) — even if you have budget left, a handoff here gives the evaluator and the next task a clean slate.
