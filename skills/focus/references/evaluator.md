...
Spawn a fresh sub-agent using Claude Code's Agent / Task primitive, **with the model override `model: fable`** — the evaluator must never inherit the generator's model (a session hinted down to a cheaper model would otherwise get an equally cheap evaluator grading its own tier's work; judgment runs on the strongest model available). If the harness rejects `fable` (older Claude Code, restricted `availableModels`), fall back to `opus` — never sonnet or below. Sub-agents cannot resolve slash commands — point it at the command **file**, and locate it first (the path differs by install): try `.claude/commands/focus/evaluate.md` (project-vendored — the only path that exists on a fresh clone/VPS), then `~/.claude/commands/focus/evaluate.md`; if both absent (plugin install), find it with `find ~/.claude/plugins -name evaluate.md -path '*focus*' 2>/dev/null | head -1`.
